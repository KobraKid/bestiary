...
export default async function importDqTact(event: IpcMainInvokeEvent): Promise<void> {
    ...
    const pkg = await Package.findOneAndUpdate({ ns: BuiltInImporter.dqtact }, {
        name: getImporterName(BuiltInImporter.dqtact),
        ns: BuiltInImporter.dqtact,
        icon: "icon.webp",
        collections: [
...
async function __import_dqtact_download_images(imgResources: { img: string, baseURL: string, collection: string }[]) {
    for (const resource of imgResources) {
        try {
            await mkdir(path.join(envPaths('Bestiary', { suffix: '' }).data, BuiltInImporter.dqtact, 'images', resource.collection), { recursive: true });
        } catch (err) {
            if (!(err as Error).message.startsWith('EEXIST')) { console.log((err as Error).message); }
        } finally {
            // check if the file exists
            try {
                await fs.stat(path.join(envPaths('Bestiary', { suffix: '' }).data, BuiltInImporter.dqtact, 'images', resource.collection, resource.img));
                ...
                try {
                    ...
                    // write to disk
                    await fs.writeFile(path.join(envPaths('Bestiary', { suffix: '' }).data, BuiltInImporter.dqtact, 'images', resource.collection, resource.img), Buffer.from(imgArrayBuffer));
                }
                catch (err) {
